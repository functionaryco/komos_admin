defmodule Kaffy.ResourceAdmin do
  ...
  def form_fields(resource, product_id) do
    schema = resource[:schema]
  ...
  def customer_update_changeset(resource, entry, changes) do
    schema = resource[:schema]
    functions = schema.__info__(:functions)
  ...
  def clear_cart(conn, resource, default) do
    Utils.get_assigned_value_or_default(
      resource,
      :clear_cart,
      default,
